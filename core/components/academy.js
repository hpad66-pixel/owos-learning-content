...
(function(){
  ...
  R.cpmsim=function(el){var c=cfgOf(el);var tasks=c.tasks||[];var unit=c.unit||'weeks';
    ...
    var BW=114,BH=64,GX=48,GY=22,pos={},maxRow=0;
    Object.keys(cols).forEach(function(cn){cols[cn].forEach(function(id,row){pos[id]={x:(+cn)*(BW+GX),y:row*(BH+GY)};if(row>maxRow)maxRow=row;});});
    var maxCol=Math.max.apply(null,tasks.map(function(t){return rank[t.id];}));
    var cw=(maxCol+1)*(BW+GX)-GX,ch=(maxRow+1)*(BH+GY)-GY+12;
    // build step list
    var steps=[];
