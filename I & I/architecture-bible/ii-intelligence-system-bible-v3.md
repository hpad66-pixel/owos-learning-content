# PumpOS and I&I Intelligence System Bible
...
A formula is a controlled answer to one question. It is not an isolated line of mathematics. Each section below explains the physical meaning, calculation purpose, input lineage, result meaning, larger dependency, agent use, and inference boundary before preserving the registry expression.
...
The application calculates this value because a later interpretation, comparison, or engineering calculation needs this physical quantity on a controlled basis. Leaving it implicit would force a developer, spreadsheet, or agent to recreate the method without a shared contract.
...
The presence of an input in a source system does not make it acceptable. Identity, unit, time, boundary, quality, applicability, and source version must pass before the input snapshot is frozen.
...
The expression above is the executable mathematical core. The surrounding method contract controls units, domains, time alignment, interpolation, failure behavior, and numerical tolerance. Those controls are part of the calculation even when they are not visible in the equation.
...
The current registry declares no formal upstream formula dependency for this record. That does not mean the calculation is isolated; the larger method connection is stated next and must become an explicit dependency wherever production orchestration requires it.

Its rainfall-volume output becomes the denominator for capture fraction and the volume basis for RTK component allocation.

Its worked-example results appear on the dashboard through `M-02` (Rainfall volume over basin: **55.612 MG**), `M-08` (Rainfall capture fraction: **3.20%**).

Those displayed values carry the formula forward into these decisions: Supports event normalization, not a claim that all rainfall reached the sewer. Supports event comparison and RTK calibration; does not locate defects.

### How the agent and human reviewer use it

The deterministic service runs the formula. Droobi may retrieve the result, explain the inputs and assumptions, compare it with other governed results, identify missing evidence, and draft a next step. A qualified person decides whether the result is fit for the intended engineering, operating, financial, or jurisdictional use.

### What must not be inferred

- Do not treat the output as a broader diagnosis, design approval, operating command, compliance finding, or project authorization unless a separate approved method and reviewer establish that claim.
...
Its integrated volume is used wherever a changing flow series must become gallons, including RDII event volume, storage routing, and energy-time integration.

Its worked-example results appear on the dashboard through `M-07` (RDII event volume: **1.780 MG**), `M-15` (One-pump normal required storage: **0 gal**), `M-17` (Derated one-pump required storage: **75,312 gal**).

Those displayed values carry the formula forward into these decisions: Supports event comparison, storage consequence, and rehabilitation scenarios. Supports contingency comparison, not proof of all-condition adequacy. Compares required storage with usable storage.
...
Its baseline feeds RDII residual calculation, wet-weather ratios, peak-total composition, and the station inflow story.

Its worked-example results appear on the dashboard through `M-03` (Average dry-weather flow: **1.250 MGD**), `M-10` (Peak total station inflow: **3.929 MGD / 2,728.3 gpm**).

Those displayed values carry the formula forward into these decisions: Supports event residuals, ratios, and baseline comparison. Primary inflow used for the sample station-capacity comparison.
...
Its residual time series feeds event-volume integration and provides the observed hydrograph against which RTK response can be calibrated.

Its worked-example results appear on the dashboard through `M-07` (RDII event volume: **1.780 MG**).

Those displayed values carry the formula forward into these decisions: Supports event comparison, storage consequence, and rehabilitation scenarios.
...
The registry explicitly declares F-FLOW-001, F-RDII-001 as upstream dependencies. Those upstream results become controlled derived inputs. Their full precision and provenance must be retained.

Its event volume feeds rainfall capture, storage consequence, rehabilitation scenarios, and event comparison.

Its worked-example results appear on the dashboard through `M-07` (RDII event volume: **1.780 MG**), `M-08` (Rainfall capture fraction: **3.20%**).

Those displayed values carry the formula forward into these decisions: Supports event comparison, storage consequence, and rehabilitation scenarios. Supports event comparison and RTK calibration; does not locate defects.
...
The registry explicitly declares F-UNIT-001, F-RDII-002 as upstream dependencies. Those upstream results become controlled derived inputs. Their full precision and provenance must be retained.

Its capture fraction feeds event comparison and RTK calibration review while remaining separate from defect diagnosis.

Its worked-example results appear on the dashboard through `M-08` (Rainfall capture fraction: **3.20%**).

Those displayed values carry the formula forward into these decisions: Supports event comparison and RTK calibration; does not locate defects.
...
Its normalized result feeds basin screening and prioritization, but no station hydraulic calculation should use it as an inflow.

Its worked-example results appear on the dashboard through `M-06` (Normalized dry-weather GWI: **606.8 gpd/in-mi**).

Those displayed values carry the formula forward into these decisions: Supports comparison only; no universal pass or fail conclusion.
...
Its ratio feeds event-strength screening and comparison, while actual peak flow continues separately into hydraulic analysis.

The worked dashboard does not expose this formula as a standalone numbered value. It remains an intermediate, supporting, uncertainty, verification, or alternative-method calculation.
...
Its component volume and duration are derived inputs to F-RTK-002, which builds each triangular component hydrograph.

Its worked-example results appear on the dashboard through `M-07` (RDII event volume: **1.780 MG**), `M-09` (Peak RDII flow: **2.704 MGD**).

Those displayed values carry the formula forward into these decisions: Supports event comparison, storage consequence, and rehabilitation scenarios. Feeds peak total flow and station consequence.
...
Its component ordinates are derived inputs to F-RTK-003, which shifts and adds them across the rainfall series.
...
Its complete RDII hydrograph is added to expected dry-weather flow on the same clock, creating total station inflow for pump and storage analysis.

Its worked-example results appear on the dashboard through `M-07` (RDII event volume: **1.780 MG**), `M-09` (Peak RDII flow: **2.704 MGD**), `M-10` (Peak total station inflow: **3.929 MGD / 2,728.3 gpm**).

Those displayed values carry the formula forward into these decisions: Supports event comparison, storage consequence, and rehabilitation scenarios. Feeds peak total flow and station consequence. Primary inflow used for the sample station-capacity comparison.
...
Its conservation rule is the foundation for dynamic storage routing, overflow accounting, and reconciliation of unexplained volume.

Its worked-example results appear on the dashboard through `M-15` (One-pump normal required storage: **0 gal**), `M-17` (Derated one-pump required storage: **75,312 gal**).

Those displayed values carry the formula forward into these decisions: Supports contingency comparison, not proof of all-condition adequacy. Compares required storage with usable storage.
...
Its gravity-capacity estimate can support bounded upstream screening or external model checks before flow reaches the station analysis.
...
Its annual gross marginal benefit becomes a cash-flow input to present-value, net-present-value, benefit-cost, and payback screens.

Its worked-example results appear on the dashboard through `M-28` (Annual gross marginal-cost benefit: **$22,826/yr**), `M-29` (Annual net direct benefit: **-$97,174/yr**).

Those displayed values carry the formula forward into these decisions: Feeds present value and must show excluded benefits. Prevents the agent from forcing a favorable business case.
...
Its discounted benefit and cost totals feed net present value and benefit-cost ratio.

Its worked-example results appear on the dashboard through `M-30` (Present value of gross benefits: **$339,586**), `M-31` (Present value of total costs: **$10,285,297**), `M-32` (Net present value: **-$9,945,711**).

Those displayed values carry the formula forward into these decisions: Feeds NPV and benefit-cost ratio. Supports economic screening with excluded benefits shown separately.
...
Its ratio feeds the economic comparison presented to engineering, finance, and capital planning.

Its worked-example results appear on the dashboard through `M-33` (Benefit-cost ratio: **0.033**).

Those displayed values carry the formula forward into these decisions: Screens the stated scenario; it does not include omitted public or capacity benefits.
...
Its covariance estimate attaches uncertainty to selected outputs so dashboards and decisions do not present false precision.
...
Its weather-normalized result closes the loop by comparing post-work evidence with the counterfactual used to judge outcome.

Its worked-example results appear on the dashboard through `M-27` (Annual modeled I&I reduction: **35.116 MG/yr**).

Those displayed values carry the formula forward into these decisions: Feeds the narrow direct-cost screen; it is not measured removal.
...
Its converted values allow otherwise compatible formulas to exchange flow without silently mixing gpm, gpd, MGD, and cfs.

Its worked-example results appear on the dashboard through `M-04` (Groundwater infiltration: **0.250 MGD**), `M-09` (Peak RDII flow: **2.704 MGD**), `M-10` (Peak total station inflow: **3.929 MGD / 2,728.3 gpm**).

Those displayed values carry the formula forward into these decisions: Supports dry-weather investigation and rehabilitation screening. Feeds peak total flow and station consequence. Primary inflow used for the sample station-capacity comparison.
...
Its groundwater-infiltration estimate feeds normalization, annualization, rehabilitation scenarios, and dry-weather investigation.

Its worked-example results appear on the dashboard through `M-04` (Groundwater infiltration: **0.250 MGD**), `M-06` (Normalized dry-weather GWI: **606.8 gpd/in-mi**).

Those displayed values carry the formula forward into these decisions: Supports dry-weather investigation and rehabilitation screening. Supports comparison only; no universal pass or fail conclusion.
...
Its inventory denominator combines with selected flow in F-NORM-001 to create an inch-diameter-mile screening value.

Its worked-example results appear on the dashboard through `M-05` (Inch-diameter-mile inventory: **412 in-mi**), `M-06` (Normalized dry-weather GWI: **606.8 gpd/in-mi**).

Those displayed values carry the formula forward into these decisions: Enables normalized comparison when scope and diameter basis match. Supports comparison only; no universal pass or fail conclusion.
...
Its velocity and Reynolds number feed friction-factor, major-loss, minor-loss, system-curve, power, and operating-point calculations.

Its worked-example results appear on the dashboard through `M-11` (One-pump operating capacity at maximum static head: **2,994.3 gpm**), `M-12` (Conservative two-pump firm capacity: **4,129.8 gpm**).

Those displayed values carry the formula forward into these decisions: Feeds one-pump storage and cycling screens. Feeds margin and utilization, not a complete station certification.
...
Its Darcy friction factor feeds F-HYD-003 major head loss and therefore the system head curve.
...
Its major head-loss result is added to static and minor head in F-HYD-005.
...
Its localized head-loss result is added to static and major head in F-HYD-005.
...
The registry explicitly declares F-HYD-001, F-HYD-002, F-HYD-003, F-HYD-004 as upstream dependencies. Those upstream results become controlled derived inputs. Their full precision and provenance must be retained.

Its system curve is intersected with the pump curve in F-PUMP-001 and supplies total dynamic head to energy calculations.
...
Its operating-point capacity feeds margin, utilization, storage deficit, cycling, operating-time, and power calculations.

Its worked-example results appear on the dashboard through `M-11` (One-pump operating capacity at maximum static head: **2,994.3 gpm**), `M-12` (Conservative two-pump firm capacity: **4,129.8 gpm**), `M-16` (Derated one-pump available capacity: **2,245.7 gpm**).

Those displayed values carry the formula forward into these decisions: Feeds one-pump storage and cycling screens. Feeds margin and utilization, not a complete station certification. Feeds dynamic storage routing.
...
Its margin and utilization feed the station condition review and determine whether contingency analysis deserves attention.

Its worked-example results appear on the dashboard through `M-13` (Peak firm-capacity margin: **1,401.5 gpm / 33.94%**), `M-14` (Peak firm-capacity utilization: **66.06%**).

Those displayed values carry the formula forward into these decisions: Supports normal-condition screening and contingency escalation. Supports comparison and prioritization with visible scenario assumptions.
...
Its time-to-exhaust result feeds contingency response planning and the Action and Approval Center.

Its worked-example results appear on the dashboard through `M-21` (Time to exhaust usable storage during full outage: **16.49 min**).

Those displayed values carry the formula forward into these decisions: Supports response planning; dynamic inflow and controls may change the result.
...
Its required-storage screen is compared with usable storage to produce the contingency shortfall used in action review.

Its worked-example results appear on the dashboard through `M-19` (Complete-outage required storage for 30 minutes: **81,848 gal**), `M-20` (Complete-outage storage shortfall: **36,848 gal**).

Those displayed values carry the formula forward into these decisions: Supports emergency-response planning with a visible conservative boundary. Supports emergency contingency review.
...
The registry explicitly declares F-MASS-001, F-FLOW-001 as upstream dependencies. Those upstream results become controlled derived inputs. Their full precision and provenance must be retained.

Its routed storage and overflow series feed dynamic resilience review, rather than being confused with the simpler constant-peak screens.

Its worked-example results appear on the dashboard through `M-15` (One-pump normal required storage: **0 gal**), `M-17` (Derated one-pump required storage: **75,312 gal**), `M-18` (Derated one-pump storage shortfall: **30,312 gal**).

Those displayed values carry the formula forward into these decisions: Supports contingency comparison, not proof of all-condition adequacy. Compares required storage with usable storage. Creates a contingency-review finding and draft investigation.
...
Its cycle result is compared with reviewed control or manual requirements and may initiate maintenance or control investigation.

Its worked-example results appear on the dashboard through `M-22` (Illustrative cycles per hour: **2.475 cycles/hr**).

Those displayed values carry the formula forward into these decisions: Supports comparison with reviewed manual or control requirements.
...
Its equivalent hours feed operating-duty comparison and, only under the applicable rule pack, the Miami-Dade NAPOT calculation.

Its worked-example results appear on the dashboard through `M-25` (Aggregate pump hours per average dry-weather day: **6.892 hr/day**), `M-26` (Illustrative Miami-Dade NAPOT: **3.446 hr/day**).

Those displayed values carry the formula forward into these decisions: Supports operating-time analysis with visible capacity assumptions. No current capacity or compliance determination without applicable legal review.
...
Its jurisdiction-specific output feeds a reviewed Miami-Dade operating-time finding, never the universal formula library.

Its worked-example results appear on the dashboard through `M-26` (Illustrative Miami-Dade NAPOT: **3.446 hr/day**).

Those displayed values carry the formula forward into these decisions: No current capacity or compliance determination without applicable legal review.
...
Its power result becomes the time-varying input to F-ENERGY-002.

Its worked-example results appear on the dashboard through `M-23` (Event pumping energy: **1,589.9 kWh**), `M-24` (Event energy cost: **$190.79**).

Those displayed values carry the formula forward into these decisions: Supports energy comparison and cost calculation. Supports event and scenario comparison, not full demand-charge accounting.
...
The registry explicitly declares F-ENERGY-001, F-FLOW-001 as upstream dependencies. Those upstream results become controlled derived inputs. Their full precision and provenance must be retained.

Its energy and cost results feed operating-burden review and rehabilitation economics when the cost scope permits.
...
Its annual volume feeds marginal-cost benefit and the declared annual rehabilitation scenario.
...
The registry explicitly declares F-PV-001 as an upstream dependency. Those upstream results become controlled derived inputs. Their full precision and provenance must be retained.

Its net present value feeds the economic screen presented alongside benefit-cost ratio and excluded benefits.

Its worked-example results appear on the dashboard through `M-32` (Net present value: **-$9,945,711**).

Those displayed values carry the formula forward into these decisions: Supports economic screening with excluded benefits shown separately.
...
Its payback result or fail-closed refusal feeds the economic dashboard without forcing a misleading recovery period.

Its worked-example results appear on the dashboard through `M-34` (Simple payback: **Not calculable because annual net benefit is nonpositive**).

Those displayed values carry the formula forward into these decisions: Blocks a false payback claim.
...
A source class explains where evidence originates before it becomes an input. Each class has a different physical meaning, authority, failure mode, and downstream consequence.

## DS-01. Flow observations

This source class is the observed hydraulic backbone. It tells the system what the sewer or station actually recorded over time.

### Why the application needs it

Without qualified flow observations, every formula that consumes this class either fails closed or becomes limited to a clearly labeled hypothetical scenario. A substitute is acceptable only when the method contract names it and preserves the substitution.

### Where it comes from

The supported systems are station influent or discharge flow meter, temporary basin flow meter, SCADA historian, approved laboratory or field export. These systems remain the original record authorities. PumpOS and I&I Intelligence preserve locators and snapshots rather than relabeling derived copies as original evidence.

### What is wrangled

The pipeline resolves these fields: `source_tag`, `timestamp`, `value`, `unit`, `meter_status`, `calibration_reference`.

Wrangling means more than changing a column name. It establishes identity, unit, time, boundary, effective date, quality, and provenance so the record can be judged for a named method.

### What must pass before use

- **canonical station, meter, and basin identity.** A wrong identity attaches valid data to the wrong meter, basin, station, or asset.
- **timestamp and time-zone resolution.** Misaligned clocks move peaks, distort integration, and break cause-and-response comparison.
- **unit resolution.** An unresolved unit can change a result by orders of magnitude while leaving the arithmetic syntactically valid.
- **gap, duplicate, range, and rate-of-change checks.** Gaps and duplicates bias integrated volume, while implausible rates can signal sensor or ingestion failure.
- **surcharge, backwater, fouling, and low-depth review where material.** These conditions can violate the measurement assumptions and bias the recorded flow.

### What it feeds and what the reader eventually sees

This source class feeds `M-03` Average dry-weather flow, `M-04` Groundwater infiltration, `M-06` Normalized dry-weather GWI, `M-07` RDII event volume, `M-08` Rainfall capture fraction, `M-09` Peak RDII flow, `M-10` Peak total station inflow, `M-13` Peak firm-capacity margin, `M-14` Peak firm-capacity utilization, `M-15` One-pump normal required storage, `M-17` Derated one-pump required storage, `M-18` Derated one-pump storage shortfall, `M-19` Complete-outage required storage for 30 minutes, `M-20` Complete-outage storage shortfall, `M-21` Time to exhaust usable storage during full outage, `M-22` Illustrative cycles per hour, `M-25` Aggregate pump hours per average dry-weather day, `M-26` Illustrative Miami-Dade NAPOT, `M-27` Annual modeled I&I reduction.

A source record never becomes a conclusion by itself. It becomes one accepted input in a named calculation, and the output retains the source reference so the dashboard can explain its origin.

---

## DS-02. Rainfall observations

This source class supplies the forcing event. Without accepted rainfall on the same clock and geography, rainfall response cannot be separated defensibly.

### Why the application needs it

Without qualified rainfall observations, every formula that consumes this class either fails closed or becomes limited to a clearly labeled hypothetical scenario. A substitute is acceptable only when the method contract names it and preserves the substitution.

### Where it comes from

The supported systems are utility rain gauge, approved public weather source, radar or gridded rainfall product, validated uploaded event file. These systems remain the original record authorities. PumpOS and I&I Intelligence preserve locators and snapshots rather than relabeling derived copies as original evidence.

### What is wrangled

The pipeline resolves these fields: `gauge_or_grid_id`, `timestamp`, `incremental_depth`, `unit`, `quality_code`, `spatial_method`.

Wrangling means more than changing a column name. It establishes identity, unit, time, boundary, effective date, quality, and provenance so the record can be judged for a named method.

### What must pass before use

- **gauge identity and coordinates.** A wrong identity attaches valid data to the wrong meter, basin, station, or asset.
- **event clock alignment.** Misaligned clocks move peaks, distort integration, and break cause-and-response comparison.
- **missing-period check.** Missing rainfall can understate storm forcing and overstate apparent capture.
- **spatial-representativeness review.** A gauge outside the effective storm pattern may not represent rainfall over the basin.

### What it feeds and what the reader eventually sees

This source class feeds `M-01` Event rainfall depth, `M-02` Rainfall volume over basin, `M-07` RDII event volume, `M-08` Rainfall capture fraction, `M-09` Peak RDII flow, `M-10` Peak total station inflow, `M-13` Peak firm-capacity margin, `M-14` Peak firm-capacity utilization, `M-27` Annual modeled I&I reduction.
...
This source class defines the physical and geographic denominator. It tells calculations which basin and sewer assets the observations represent.

### Why the application needs it

Without qualified basin and sewer inventory, every formula that consumes this class either fails closed or becomes limited to a clearly labeled hypothetical scenario. A substitute is acceptable only when the method contract names it and preserves the substitution.

### Where it comes from

The supported systems are GIS, asset registry, as-built drawings, field-verified inventory. These systems remain the original record authorities. PumpOS and I&I Intelligence preserve locators and snapshots rather than relabeling derived copies as original evidence.

### What is wrangled

The pipeline resolves these fields: `basin_boundary`, `area`, `pipe_segment_id`, `pipe_length`, `pipe_diameter`, `asset_class`, `effective_date`.

Wrangling means more than changing a column name. It establishes identity, unit, time, boundary, effective date, quality, and provenance so the record can be judged for a named method.

### What must pass before use

- **duplicate and abandoned segment resolution.** Gaps and duplicates bias integrated volume, while implausible rates can signal sensor or ingestion failure.
- **diameter basis.** Diameter controls normalization, area, velocity, Reynolds number, and head loss.
- **included asset classes.** The included classes define what the denominator represents and whether comparison is fair.
- **numerator and denominator boundary match.** A numerator and denominator from different service boundaries create a meaningless ratio or remainder.

### What it feeds and what the reader eventually sees

This source class feeds `M-02` Rainfall volume over basin, `M-05` Inch-diameter-mile inventory, `M-06` Normalized dry-weather GWI, `M-07` RDII event volume, `M-08` Rainfall capture fraction, `M-09` Peak RDII flow, `M-10` Peak total station inflow, `M-13` Peak firm-capacity margin, `M-14` Peak firm-capacity utilization, `M-27` Annual modeled I&I reduction.
...
This source class supports the sanitary-flow estimate that must be separated from groundwater and rainfall response.

### Why the application needs it

Without qualified population, customer, and sanitary-flow basis, every formula that consumes this class either fails closed or becomes limited to a clearly labeled hypothetical scenario. A substitute is acceptable only when the method contract names it and preserves the substitution.

### Where it comes from

The supported systems are billing system, water-use records, customer information system, planning records, approved industrial and commercial discharge records. These systems remain the original record authorities. PumpOS and I&I Intelligence preserve locators and snapshots rather than relabeling derived copies as original evidence.

### What is wrangled

The pipeline resolves these fields: `population`, `service_connections`, `water_use`, `customer_class`, `known_process_flow`, `analysis_period`.

Wrangling means more than changing a column name. It establishes identity, unit, time, boundary, effective date, quality, and provenance so the record can be judged for a named method.

### What must pass before use

- **same service boundary as measured flow.** A numerator and denominator from different service boundaries create a meaningless ratio or remainder.
- **non-domestic and transferred flows resolved.** Omitted industrial, commercial, or transferred flow is easily mislabeled as groundwater infiltration.
- **seasonal and day-type basis recorded.** Customer use and groundwater conditions vary, so the baseline period must represent the analysis purpose.

### What it feeds and what the reader eventually sees

This source class feeds `M-04` Groundwater infiltration, `M-06` Normalized dry-weather GWI.
...
This source class describes what the installed pumps can produce under the represented condition, speed, impeller, and revision.

### Why the application needs it

Without qualified pump performance, every formula that consumes this class either fails closed or becomes limited to a clearly labeled hypothetical scenario. A substitute is acceptable only when the method contract names it and preserves the substitution.

### Where it comes from

The supported systems are manufacturer pump curve, certified test curve, field performance test, VFD speed record, PumpOS asset registry. These systems remain the original record authorities. PumpOS and I&I Intelligence preserve locators and snapshots rather than relabeling derived copies as original evidence.

### What is wrangled

The pipeline resolves these fields: `pump_id`, `speed`, `flow`, `head`, `pump_efficiency`, `motor_efficiency`, `curve_revision`.

Wrangling means more than changing a column name. It establishes identity, unit, time, boundary, effective date, quality, and provenance so the record can be judged for a named method.

### What must pass before use

- **installed pump and curve applicability.** The wrong pump, speed, impeller, or revision produces the wrong operating point.
- **speed and impeller configuration.** Pump performance changes with configuration, so the selected curve must match the installed condition.
- **interpolation boundary.** Uncontrolled extrapolation can invent pump performance outside the evidence in the curve.
- **test or manufacturer provenance.** Resolving this condition prevents an ambiguous record from becoming a confident downstream result.

### What it feeds and what the reader eventually sees

This source class feeds `M-11` One-pump operating capacity at maximum static head, `M-12` Conservative two-pump firm capacity, `M-13` Peak firm-capacity margin, `M-14` Peak firm-capacity utilization, `M-15` One-pump normal required storage, `M-16` Derated one-pump available capacity, `M-17` Derated one-pump required storage, `M-18` Derated one-pump storage shortfall, `M-22` Illustrative cycles per hour, `M-23` Event pumping energy, `M-24` Event energy cost, `M-25` Aggregate pump hours per average dry-weather day, `M-26` Illustrative Miami-Dade NAPOT.
...
This source class describes the connected hydraulic resistance and static lift that the pumps must overcome.

### Why the application needs it

Without qualified force-main and hydraulic geometry, every formula that consumes this class either fails closed or becomes limited to a clearly labeled hypothetical scenario. A substitute is acceptable only when the method contract names it and preserves the substitution.

### Where it comes from

The supported systems are GIS, as-built drawings, survey, design record, field inspection. These systems remain the original record authorities. PumpOS and I&I Intelligence preserve locators and snapshots rather than relabeling derived copies as original evidence.

### What is wrangled

The pipeline resolves these fields: `internal_diameter`, `length`, `roughness`, `fitting_and_valve_loss_coefficients`, `static_head_range`, `fluid_properties`.

Wrangling means more than changing a column name. It establishes identity, unit, time, boundary, effective date, quality, and provenance so the record can be judged for a named method.

### What must pass before use

- **current configuration and valve state.** The system curve changes when the hydraulic lineup changes.
- **internal-diameter basis.** Diameter controls normalization, area, velocity, Reynolds number, and head loss.
- **elevation datum.** Mixed elevation datums produce a false static head.
- **roughness and fluid-property basis.** These assumptions control friction and can materially move the operating point.

### What it feeds and what the reader eventually sees

This source class feeds `M-11` One-pump operating capacity at maximum static head, `M-12` Conservative two-pump firm capacity, `M-13` Peak firm-capacity margin, `M-14` Peak firm-capacity utilization, `M-15` One-pump normal required storage, `M-16` Derated one-pump available capacity, `M-17` Derated one-pump required storage, `M-18` Derated one-pump storage shortfall, `M-23` Event pumping energy, `M-24` Event energy cost.
...
This source class defines how level becomes volume and how controls change available pumping and response time.

### Why the application needs it

Without qualified wet-well and control data, every formula that consumes this class either fails closed or becomes limited to a clearly labeled hypothetical scenario. A substitute is acceptable only when the method contract names it and preserves the substitution.

### Where it comes from

The supported systems are SCADA, control narrative, wet-well drawings, level survey, operator-verified settings. These systems remain the original record authorities. PumpOS and I&I Intelligence preserve locators and snapshots rather than relabeling derived copies as original evidence.

### What is wrangled

The pipeline resolves these fields: `level`, `level_volume_curve`, `pump_on_and_off_levels`, `high_alarm_level`, `overflow_level`, `usable_storage`, `pump_state`, `response_time`.

Wrangling means more than changing a column name. It establishes identity, unit, time, boundary, effective date, quality, and provenance so the record can be judged for a named method.

### What must pass before use

- **current settings and effective date.** An obsolete control setting describes a station that no longer exists operationally.
- **level-sensor validation.** A biased level reading changes storage, controls, alarms, and response time.
- **usable-storage boundary.** Storage must be measured between the actual starting and limiting levels used by the scenario.
- **control-mode applicability.** Resolving this condition prevents an ambiguous record from becoming a confident downstream result.

### What it feeds and what the reader eventually sees

This source class feeds `M-15` One-pump normal required storage, `M-17` Derated one-pump required storage, `M-18` Derated one-pump storage shortfall, `M-19` Complete-outage required storage for 30 minutes, `M-20` Complete-outage storage shortfall, `M-21` Time to exhaust usable storage during full outage, `M-22` Illustrative cycles per hour.
...
This source class connects hydraulic operation with measured or modeled electrical burden and the applicable price structure.

### Why the application needs it

Without qualified electrical energy and tariff, every formula that consumes this class either fails closed or becomes limited to a clearly labeled hypothetical scenario. A substitute is acceptable only when the method contract names it and preserves the substitution.

### Where it comes from

The supported systems are power meter, VFD, motor-control center, SCADA historian, utility tariff and finance records. These systems remain the original record authorities. PumpOS and I&I Intelligence preserve locators and snapshots rather than relabeling derived copies as original evidence.

### What is wrangled

The pipeline resolves these fields: `kW`, `kWh`, `power_factor`, `demand`, `tariff`, `tariff_period`.

Wrangling means more than changing a column name. It establishes identity, unit, time, boundary, effective date, quality, and provenance so the record can be judged for a named method.

### What must pass before use

- **meter-to-asset association.** A wrong identity attaches valid data to the wrong meter, basin, station, or asset.
- **interval alignment.** Misaligned clocks move peaks, distort integration, and break cause-and-response comparison.
- **demand and time-of-use treatment.** Energy-only pricing can omit material demand or time-of-use cost.
- **tariff effective date.** An obsolete control setting describes a station that no longer exists operationally.

### What it feeds and what the reader eventually sees

This source class feeds `M-23` Event pumping energy, `M-24` Event energy cost.
...
This source class gives the economic scenario its price year, cost scope, assumed reduction, and discount basis.

### Why the application needs it

Without qualified cost and rehabilitation scenario, every formula that consumes this class either fails closed or becomes limited to a clearly labeled hypothetical scenario. A substitute is acceptable only when the method contract names it and preserves the substitution.

### Where it comes from

The supported systems are finance system, capital estimate, work-order history, treatment and conveyance marginal-cost study, approved planning assumptions. These systems remain the original record authorities. PumpOS and I&I Intelligence preserve locators and snapshots rather than relabeling derived copies as original evidence.

### What is wrangled

The pipeline resolves these fields: `capital_cost`, `annual_operations_and_maintenance_cost`, `marginal_cost_per_million_gallons`, `discount_rate`, `useful_life`, `assumed_reduction`.
...
- **real or nominal basis.** The discount rate and cash flows must use the same inflation basis.
- **marginal versus average cost.** Only avoidable marginal cost belongs in a direct savings calculation unless another scope is approved.
- **excluded-benefit disclosure.** Visible exclusions prevent a narrow cost screen from being mistaken for total public value.

### What it feeds and what the reader eventually sees

This source class feeds `M-27` Annual modeled I&I reduction, `M-28` Annual gross marginal-cost benefit, `M-29` Annual net direct benefit, `M-30` Present value of gross benefits, `M-31` Present value of total costs, `M-32` Net present value, `M-33` Benefit-cost ratio, `M-34` Simple payback.
...
This source class supplies current jurisdiction-specific authority without contaminating the universal formula layer.

### Why the application needs it

Without qualified jurisdiction and regulatory evidence, every formula that consumes this class either fails closed or becomes limited to a clearly labeled hypothetical scenario. A substitute is acceptable only when the method contract names it and preserves the substitution.

### Where it comes from

The supported systems are RegOS, official federal instrument, approved jurisdiction rule pack. These systems remain the original record authorities. PumpOS and I&I Intelligence preserve locators and snapshots rather than relabeling derived copies as original evidence.

### What is wrangled

The pipeline resolves these fields: `authority`, `locator`, `effective_date`, `geography`, `regulated_entity`, `metric_definition`, `exceptions`.

Wrangling means more than changing a column name. It establishes identity, unit, time, boundary, effective date, quality, and provenance so the record can be judged for a named method.

### What must pass before use

- **current legal-status review.** A superseded or withdrawn instrument cannot support a current requirement.
- **applicability review.** A valid rule still cannot be applied to the wrong geography, entity, asset, or fact pattern.
- **separation from universal mathematics.** Jurisdiction-specific rules must not be presented as universal engineering formulas.

### What it feeds and what the reader eventually sees

This source class feeds `M-26` Illustrative Miami-Dade NAPOT.
...
This source class connects approved manufacturer and utility knowledge with the exact asset, requirement, task, and completion evidence.

### Why the application needs it

Without qualified manual and maintenance evidence, every formula that consumes this class either fails closed or becomes limited to a clearly labeled hypothetical scenario. A substitute is acceptable only when the method contract names it and preserves the substitution.

### Where it comes from

The supported systems are versioned object storage, Knowledge Intake Workbench, CMMS, PumpOS asset registry. These systems remain the original record authorities. PumpOS and I&I Intelligence preserve locators and snapshots rather than relabeling derived copies as original evidence.

### What is wrangled

The pipeline resolves these fields: `document_id`, `revision`, `source_locator`, `applicable_asset`, `approved_requirement`, `completion_evidence`.

Wrangling means more than changing a column name. It establishes identity, unit, time, boundary, effective date, quality, and provenance so the record can be judged for a named method.

### What must pass before use

- **exact passage locator.** The exact passage lets a reviewer verify that the requirement was extracted without changing its meaning.
...
- **supersession status.** A superseded manual must remain historical evidence, not current operating authority.

### What it feeds and what the reader eventually sees

No numbered sample dashboard metric currently consumes this source class directly.
...
The dashboard is the visible end of a calculation, not a separate truth. Each numbered value below explains what the reader sees, why it matters, where it comes from, what it feeds, and what decision boundary remains.

## M-01 / #1. Event rainfall depth

The worked dashboard displays **3.20 in**. This is classified as `measured_or_sample_input` rather than being shown as an unexplained number.

### Where the value begins

The lineage begins with DS-02. The exact stored result is `hydrograph_summary.rainfall_depth_in`. The formula chain is Direct accepted input.

This source-to-result path matters because the display may be rounded, reformatted, or combined with a label. The application must always be able to return to the stored result and the frozen input snapshot.

### What the value means and why it matters

Establishes the storm forcing used by the event calculation.

The value is calculated or displayed at this point in the story because a downstream reader needs to understand either the event forcing, the separated I&I response, the pump-station consequence, the operating burden, or the economic scenario.

### What it feeds

Confirms which event and rainfall magnitude the analysis represents.

The agent may explain this value and assemble its evidence. It may not promote the value into a broader claim than its evidence class and decision use allow.

---

## M-02 / #2. Rainfall volume over basin

The worked dashboard displays **55.612 MG**. This is classified as `calculated` rather than being shown as an unexplained number.

### Where the value begins

The lineage begins with DS-02, DS-03. The exact stored result is `hydrograph_summary.rainfall_volume_gal`. The formula chain is F-UNIT-001.

This source-to-result path matters because the display may be rounded, reformatted, or combined with a label. The application must always be able to return to the stored result and the frozen input snapshot.

### What the value means and why it matters

Provides the denominator for rainfall capture and RTK component volume.

The value is calculated or displayed at this point in the story because a downstream reader needs to understand either the event forcing, the separated I&I response, the pump-station consequence, the operating burden, or the economic scenario.

### What it feeds

Supports event normalization, not a claim that all rainfall reached the sewer.

The agent may explain this value and assemble its evidence. It may not promote the value into a broader claim than its evidence class and decision use allow.

---

## M-03 / #3. Average dry-weather flow

The worked dashboard displays **1.250 MGD**. This is classified as `calculated_baseline` rather than being shown as an unexplained number.

### Where the value begins

The lineage begins with DS-01. The exact stored result is `hydrograph_summary.average_dry_weather_flow_MGD`. The formula chain is F-DWF-001.

This source-to-result path matters because the display may be rounded, reformatted, or combined with a label. The application must always be able to return to the stored result and the frozen input snapshot.

### What the value means and why it matters

Establishes the ordinary comparison flow before rainfall response is isolated.

The value is calculated or displayed at this point in the story because a downstream reader needs to understand either the event forcing, the separated I&I response, the pump-station consequence, the operating burden, or the economic scenario.

### What it feeds

Supports event residuals, ratios, and baseline comparison.

The agent may explain this value and assemble its evidence. It may not promote the value into a broader claim than its evidence class and decision use allow.

---

## M-04 / #4. Groundwater infiltration

The worked dashboard displays **0.250 MGD**. This is classified as `calculated_method_dependent` rather than being shown as an unexplained number.

### Where the value begins

The lineage begins with DS-01, DS-04. The exact stored result is `inventory_and_dry_weather.GWI_gpd`. The formula chain is F-GWI-001 -> F-CONV-001.

This source-to-result path matters because the display may be rounded, reformatted, or combined with a label. The application must always be able to return to the stored result and the frozen input snapshot.

### What the value means and why it matters

Separates persistent groundwater contribution from base wastewater.

The value is calculated or displayed at this point in the story because a downstream reader needs to understand either the event forcing, the separated I&I response, the pump-station consequence, the operating burden, or the economic scenario.

### What it feeds

Supports dry-weather investigation and rehabilitation screening.

The agent may explain this value and assemble its evidence. It may not promote the value into a broader claim than its evidence class and decision use allow.

---

## M-05 / #5. Inch-diameter-mile inventory

The worked dashboard displays **412 in-mi**. This is classified as `calculated_inventory` rather than being shown as an unexplained number.

### Where the value begins

The lineage begins with DS-03. The exact stored result is `inventory_and_dry_weather.inch_diameter_mile`. The formula chain is F-IDM-001.

This source-to-result path matters because the display may be rounded, reformatted, or combined with a label. The application must always be able to return to the stored result and the frozen input snapshot.

### What the value means and why it matters

Provides the declared sewer-inventory denominator.

The value is calculated or displayed at this point in the story because a downstream reader needs to understand either the event forcing, the separated I&I response, the pump-station consequence, the operating burden, or the economic scenario.

### What it feeds

Enables normalized comparison when scope and diameter basis match.

The agent may explain this value and assemble its evidence. It may not promote the value into a broader claim than its evidence class and decision use allow.

---

## M-06 / #6. Normalized dry-weather GWI

The worked dashboard displays **606.8 gpd/in-mi**. This is classified as `calculated_screening_metric` rather than being shown as an unexplained number.

### Where the value begins

The lineage begins with DS-01, DS-03, DS-04. The exact stored result is `inventory_and_dry_weather.GWI_gpd_per_inch_diameter_mile`. The formula chain is F-GWI-001 -> F-IDM-001 -> F-NORM-001.

This source-to-result path matters because the display may be rounded, reformatted, or combined with a label. The application must always be able to return to the stored result and the frozen input snapshot.

### What the value means and why it matters

Normalizes the selected flow by declared pipe inventory.
...
## M-07 / #7. RDII event volume

The worked dashboard displays **1.780 MG**. This is classified as `modeled_and_integrated` rather than being shown as an unexplained number.

### Where the value begins

The lineage begins with DS-01, DS-02, DS-03. The exact stored result is `hydrograph_summary.integrated_RDII_volume_gal`. The formula chain is F-RDII-001 -> F-RDII-002 -> F-FLOW-001 -> F-RTK-001 -> F-RTK-002 -> F-RTK-003.

This source-to-result path matters because the display may be rounded, reformatted, or combined with a label. The application must always be able to return to the stored result and the frozen input snapshot.

### What the value means and why it matters

Quantifies the event-related volume above the expected baseline.

The value is calculated or displayed at this point in the story because a downstream reader needs to understand either the event forcing, the separated I&I response, the pump-station consequence, the operating burden, or the economic scenario.

### What it feeds

Supports event comparison, storage consequence, and rehabilitation scenarios.

The agent may explain this value and assemble its evidence. It may not promote the value into a broader claim than its evidence class and decision use allow.

---

## M-08 / #8. Rainfall capture fraction

The worked dashboard displays **3.20%**. This is classified as `calculated_ratio` rather than being shown as an unexplained number.

### Where the value begins

The lineage begins with DS-01, DS-02, DS-03. The exact stored result is `hydrograph_summary.capture_fraction_total_R`. The formula chain is F-UNIT-001 -> F-RDII-002 -> F-RDII-003.

This source-to-result path matters because the display may be rounded, reformatted, or combined with a label. The application must always be able to return to the stored result and the frozen input snapshot.

### What the value means and why it matters

Relates RDII volume to rainfall volume over the same boundary.

The value is calculated or displayed at this point in the story because a downstream reader needs to understand either the event forcing, the separated I&I response, the pump-station consequence, the operating burden, or the economic scenario.

### What it feeds

Supports event comparison and RTK calibration; does not locate defects.

The agent may explain this value and assemble its evidence. It may not promote the value into a broader claim than its evidence class and decision use allow.

---

## M-09 / #9. Peak RDII flow

The worked dashboard displays **2.704 MGD**. This is classified as `modeled` rather than being shown as an unexplained number.

### Where the value begins

The lineage begins with DS-01, DS-02, DS-03. The exact stored result is `hydrograph_summary.peak_RDII_MGD`. The formula chain is F-RTK-001 -> F-RTK-002 -> F-RTK-003 -> F-CONV-001.

This source-to-result path matters because the display may be rounded, reformatted, or combined with a label. The application must always be able to return to the stored result and the frozen input snapshot.

### What the value means and why it matters

Identifies the modeled event-related peak added to the station load.

The value is calculated or displayed at this point in the story because a downstream reader needs to understand either the event forcing, the separated I&I response, the pump-station consequence, the operating burden, or the economic scenario.

### What it feeds

Feeds peak total flow and station consequence.

The agent may explain this value and assemble its evidence. It may not promote the value into a broader claim than its evidence class and decision use allow.

---

## M-10 / #10. Peak total station inflow

The worked dashboard displays **3.929 MGD / 2,728.3 gpm**. This is classified as `modeled_total` rather than being shown as an unexplained number.

### Where the value begins

The lineage begins with DS-01, DS-02, DS-03. The exact stored result is `hydrograph_summary.peak_total_flow_gpm`. The formula chain is F-DWF-001 -> F-RTK-003 -> F-CONV-001.

This source-to-result path matters because the display may be rounded, reformatted, or combined with a label. The application must always be able to return to the stored result and the frozen input snapshot.

### What the value means and why it matters

Carries basin response into pump, storage, and contingency analysis.

The value is calculated or displayed at this point in the story because a downstream reader needs to understand either the event forcing, the separated I&I response, the pump-station consequence, the operating burden, or the economic scenario.

### What it feeds

Primary inflow used for the sample station-capacity comparison.

The agent may explain this value and assemble its evidence. It may not promote the value into a broader claim than its evidence class and decision use allow.

---

## M-11 / #11. One-pump operating capacity at maximum static head

The worked dashboard displays **2,994.3 gpm**. This is classified as `calculated_operating_point` rather than being shown as an unexplained number.

### Where the value begins

The lineage begins with DS-05, DS-06. The exact stored result is `pump_station_analysis.operating_points.1_pump_maximum_static_head.total_flow_gpm`. The formula chain is F-HYD-001 -> F-HYD-002 -> F-HYD-003 -> F-HYD-004 -> F-HYD-005 -> F-PUMP-001.

This source-to-result path matters because the display may be rounded, reformatted, or combined with a label. The application must always be able to return to the stored result and the frozen input snapshot.

### What the value means and why it matters

Represents conservative one-pump availability for the sample curves and head.

The value is calculated or displayed at this point in the story because a downstream reader needs to understand either the event forcing, the separated I&I response, the pump-station consequence, the operating burden, or the economic scenario.

### What it feeds

Feeds one-pump storage and cycling screens.

The agent may explain this value and assemble its evidence. It may not promote the value into a broader claim than its evidence class and decision use allow.

---

## M-12 / #12. Conservative two-pump firm capacity

The worked dashboard displays **4,129.8 gpm**. This is classified as `calculated_operating_point` rather than being shown as an unexplained number.

### Where the value begins

The lineage begins with DS-05, DS-06. The exact stored result is `pump_station_analysis.capacity.conservative_firm_capacity_gpm`. The formula chain is F-HYD-001 -> F-HYD-002 -> F-HYD-003 -> F-HYD-004 -> F-HYD-005 -> F-PUMP-001.

This source-to-result path matters because the display may be rounded, reformatted, or combined with a label. The application must always be able to return to the stored result and the frozen input snapshot.

### What the value means and why it matters

Provides the available flow for the normal firm-capacity comparison.

The value is calculated or displayed at this point in the story because a downstream reader needs to understand either the event forcing, the separated I&I response, the pump-station consequence, the operating burden, or the economic scenario.

### What it feeds

Feeds margin and utilization, not a complete station certification.

The agent may explain this value and assemble its evidence. It may not promote the value into a broader claim than its evidence class and decision use allow.

---

## M-13 / #13. Peak firm-capacity margin

The worked dashboard displays **1,401.5 gpm / 33.94%**. This is classified as `calculated_comparison` rather than being shown as an unexplained number.

### Where the value begins

The lineage begins with DS-01, DS-02, DS-03, DS-05, DS-06. The exact stored result is `pump_station_analysis.capacity.peak_margin_gpm`. The formula chain is F-PUMP-002.

This source-to-result path matters because the display may be rounded, reformatted, or combined with a label. The application must always be able to return to the stored result and the frozen input snapshot.

### What the value means and why it matters

Shows remaining modeled flow margin under the stated normal scenario.

The value is calculated or displayed at this point in the story because a downstream reader needs to understand either the event forcing, the separated I&I response, the pump-station consequence, the operating burden, or the economic scenario.

### What it feeds

Supports normal-condition screening and contingency escalation.

The agent may explain this value and assemble its evidence. It may not promote the value into a broader claim than its evidence class and decision use allow.

---

## M-14 / #14. Peak firm-capacity utilization

The worked dashboard displays **66.06%**. This is classified as `calculated_ratio` rather than being shown as an unexplained number.

### Where the value begins

The lineage begins with DS-01, DS-02, DS-03, DS-05, DS-06. The exact stored result is `pump_station_analysis.capacity.peak_utilization_fraction`. The formula chain is F-PUMP-002.

This source-to-result path matters because the display may be rounded, reformatted, or combined with a label. The application must always be able to return to the stored result and the frozen input snapshot.

### What the value means and why it matters

Shows how much of the stated available capacity the event uses.

The value is calculated or displayed at this point in the story because a downstream reader needs to understand either the event forcing, the separated I&I response, the pump-station consequence, the operating burden, or the economic scenario.

### What it feeds

Supports comparison and prioritization with visible scenario assumptions.

The agent may explain this value and assemble its evidence. It may not promote the value into a broader claim than its evidence class and decision use allow.

---

## M-15 / #15. One-pump normal required storage

The worked dashboard displays **0 gal**. This is classified as `modeled_contingency` rather than being shown as an unexplained number.

### Where the value begins

The lineage begins with DS-01, DS-05, DS-06, DS-07. The exact stored result is `pump_station_analysis.storage.contingency_results.ONE-PUMP-NORMAL.required_storage_gal`. The formula chain is F-MASS-001 -> F-FLOW-001 -> F-PUMP-007.

This source-to-result path matters because the display may be rounded, reformatted, or combined with a label. The application must always be able to return to the stored result and the frozen input snapshot.

### What the value means and why it matters

Shows whether the event creates a storage deficit with one normal pump.

The value is calculated or displayed at this point in the story because a downstream reader needs to understand either the event forcing, the separated I&I response, the pump-station consequence, the operating burden, or the economic scenario.

### What it feeds

Supports contingency comparison, not proof of all-condition adequacy.

The agent may explain this value and assemble its evidence. It may not promote the value into a broader claim than its evidence class and decision use allow.

---

## M-16 / #16. Derated one-pump available capacity

The worked dashboard displays **2,245.7 gpm**. This is classified as `hypothetical_scenario` rather than being shown as an unexplained number.

### Where the value begins

The lineage begins with DS-05, DS-06. The exact stored result is `pump_station_analysis.storage.contingency_results.ONE-PUMP-DERATED-75.available_capacity_gpm`. The formula chain is F-PUMP-001.

This source-to-result path matters because the display may be rounded, reformatted, or combined with a label. The application must always be able to return to the stored result and the frozen input snapshot.

### What the value means and why it matters

Defines the available pumping assumption for the derated contingency.

The value is calculated or displayed at this point in the story because a downstream reader needs to understand either the event forcing, the separated I&I response, the pump-station consequence, the operating burden, or the economic scenario.

### What it feeds

Feeds dynamic storage routing.

The agent may explain this value and assemble its evidence. It may not promote the value into a broader claim than its evidence class and decision use allow.

---

## M-17 / #17. Derated one-pump required storage

The worked dashboard displays **75,312 gal**. This is classified as `modeled_contingency` rather than being shown as an unexplained number.

### Where the value begins

The lineage begins with DS-01, DS-05, DS-06, DS-07. The exact stored result is `pump_station_analysis.storage.contingency_results.ONE-PUMP-DERATED-75.required_storage_gal`. The formula chain is F-MASS-001 -> F-FLOW-001 -> F-PUMP-007.

This source-to-result path matters because the display may be rounded, reformatted, or combined with a label. The application must always be able to return to the stored result and the frozen input snapshot.

### What the value means and why it matters

Shows the maximum modeled accumulated deficit during the event.

The value is calculated or displayed at this point in the story because a downstream reader needs to understand either the event forcing, the separated I&I response, the pump-station consequence, the operating burden, or the economic scenario.

### What it feeds

Compares required storage with usable storage.

The agent may explain this value and assemble its evidence. It may not promote the value into a broader claim than its evidence class and decision use allow.

---

## M-18 / #18. Derated one-pump storage shortfall

The worked dashboard displays **30,312 gal**. This is classified as `modeled_contingency` rather than being shown as an unexplained number.

### Where the value begins

The lineage begins with DS-01, DS-05, DS-06, DS-07. The exact stored result is `pump_station_analysis.storage.contingency_results.ONE-PUMP-DERATED-75.storage_shortfall_gal`. The formula chain is F-PUMP-007.

This source-to-result path matters because the display may be rounded, reformatted, or combined with a label. The application must always be able to return to the stored result and the frozen input snapshot.

### What the value means and why it matters

Quantifies modeled storage beyond the declared usable volume.

The value is calculated or displayed at this point in the story because a downstream reader needs to understand either the event forcing, the separated I&I response, the pump-station consequence, the operating burden, or the economic scenario.

### What it feeds

Creates a contingency-review finding and draft investigation.

The agent may explain this value and assemble its evidence. It may not promote the value into a broader claim than its evidence class and decision use allow.

---

## M-19 / #19. Complete-outage required storage for 30 minutes

The worked dashboard displays **81,848 gal**. This is classified as `constant_peak_screen` rather than being shown as an unexplained number.

### Where the value begins

The lineage begins with DS-01, DS-07. The exact stored result is `pump_station_analysis.storage.contingency_results.COMPLETE-OUTAGE-30-MIN.required_storage_gal`. The formula chain is F-PUMP-004.

This source-to-result path matters because the display may be rounded, reformatted, or combined with a label. The application must always be able to return to the stored result and the frozen input snapshot.

### What the value means and why it matters

Screens storage required during a stated no-pumping response interval.

The value is calculated or displayed at this point in the story because a downstream reader needs to understand either the event forcing, the separated I&I response, the pump-station consequence, the operating burden, or the economic scenario.

### What it feeds

Supports emergency-response planning with a visible conservative boundary.

The agent may explain this value and assemble its evidence. It may not promote the value into a broader claim than its evidence class and decision use allow.

---

## M-20 / #20. Complete-outage storage shortfall

The worked dashboard displays **36,848 gal**. This is classified as `constant_peak_screen` rather than being shown as an unexplained number.

### Where the value begins

The lineage begins with DS-01, DS-07. The exact stored result is `pump_station_analysis.storage.contingency_results.COMPLETE-OUTAGE-30-MIN.storage_shortfall_gal`. The formula chain is F-PUMP-004.

This source-to-result path matters because the display may be rounded, reformatted, or combined with a label. The application must always be able to return to the stored result and the frozen input snapshot.

### What the value means and why it matters

Shows the difference between required and usable storage.

The value is calculated or displayed at this point in the story because a downstream reader needs to understand either the event forcing, the separated I&I response, the pump-station consequence, the operating burden, or the economic scenario.

### What it feeds

Supports emergency contingency review.

The agent may explain this value and assemble its evidence. It may not promote the value into a broader claim than its evidence class and decision use allow.

---

## M-21 / #21. Time to exhaust usable storage during full outage

The worked dashboard displays **16.49 min**. This is classified as `constant_peak_screen` rather than being shown as an unexplained number.

### Where the value begins

The lineage begins with DS-01, DS-07. The exact stored result is `pump_station_analysis.storage.contingency_results.COMPLETE-OUTAGE-30-MIN.time_to_exhaust_available_storage_min`. The formula chain is F-PUMP-003.

This source-to-result path matters because the display may be rounded, reformatted, or combined with a label. The application must always be able to return to the stored result and the frozen input snapshot.

### What the value means and why it matters

Converts the stated flow deficit and storage into an illustrative response window.

The value is calculated or displayed at this point in the story because a downstream reader needs to understand either the event forcing, the separated I&I response, the pump-station consequence, the operating burden, or the economic scenario.

### What it feeds

Supports response planning; dynamic inflow and controls may change the result.

The agent may explain this value and assemble its evidence. It may not promote the value into a broader claim than its evidence class and decision use allow.

---

## M-22 / #22. Illustrative cycles per hour

The worked dashboard displays **2.475 cycles/hr**. This is classified as `calculated_limited_applicability` rather than being shown as an unexplained number.

### Where the value begins

The lineage begins with DS-01, DS-05, DS-07. The exact stored result is `pump_station_analysis.cycling.cycles_per_hour`. The formula chain is F-PUMP-005.

This source-to-result path matters because the display may be rounded, reformatted, or combined with a label. The application must always be able to return to the stored result and the frozen input snapshot.

### What the value means and why it matters

Screens cycling for one constant-speed pump and constant inflow.

The value is calculated or displayed at this point in the story because a downstream reader needs to understand either the event forcing, the separated I&I response, the pump-station consequence, the operating burden, or the economic scenario.

### What it feeds

Supports comparison with reviewed manual or control requirements.

The agent may explain this value and assemble its evidence. It may not promote the value into a broader claim than its evidence class and decision use allow.

---

## M-23 / #23. Event pumping energy

The worked dashboard displays **1,589.9 kWh**. This is classified as `modeled_energy` rather than being shown as an unexplained number.

### Where the value begins

The lineage begins with DS-05, DS-06, DS-08. The exact stored result is `pump_station_analysis.energy.event_staged_control_energy_kWh`. The formula chain is F-ENERGY-001 -> F-ENERGY-002.

This source-to-result path matters because the display may be rounded, reformatted, or combined with a label. The application must always be able to return to the stored result and the frozen input snapshot.

### What the value means and why it matters

Quantifies energy under the stated staged-control approximation.

The value is calculated or displayed at this point in the story because a downstream reader needs to understand either the event forcing, the separated I&I response, the pump-station consequence, the operating burden, or the economic scenario.

### What it feeds

Supports energy comparison and cost calculation.

The agent may explain this value and assemble its evidence. It may not promote the value into a broader claim than its evidence class and decision use allow.

---

## M-24 / #24. Event energy cost

The worked dashboard displays **$190.79**. This is classified as `modeled_cost` rather than being shown as an unexplained number.

### Where the value begins

The lineage begins with DS-05, DS-06, DS-08. The exact stored result is `pump_station_analysis.energy.event_staged_control_energy_cost_USD`. The formula chain is F-ENERGY-001 -> F-ENERGY-002.

This source-to-result path matters because the display may be rounded, reformatted, or combined with a label. The application must always be able to return to the stored result and the frozen input snapshot.

### What the value means and why it matters

Converts modeled energy to the stated tariff boundary.

The value is calculated or displayed at this point in the story because a downstream reader needs to understand either the event forcing, the separated I&I response, the pump-station consequence, the operating burden, or the economic scenario.

### What it feeds

Supports event and scenario comparison, not full demand-charge accounting.

The agent may explain this value and assemble its evidence. It may not promote the value into a broader claim than its evidence class and decision use allow.

---

## M-25 / #25. Aggregate pump hours per average dry-weather day

The worked dashboard displays **6.892 hr/day**. This is classified as `calculated_equivalent_hours` rather than being shown as an unexplained number.

### Where the value begins

The lineage begins with DS-01, DS-05. The exact stored result is `pump_station_analysis.operating_time.aggregate_pump_hours_per_ADWF_day`. The formula chain is F-PUMP-006.

This source-to-result path matters because the display may be rounded, reformatted, or combined with a label. The application must always be able to return to the stored result and the frozen input snapshot.

### What the value means and why it matters

Relates pumped daily volume to representative single-pump capacity.

The value is calculated or displayed at this point in the story because a downstream reader needs to understand either the event forcing, the separated I&I response, the pump-station consequence, the operating burden, or the economic scenario.

### What it feeds

Supports operating-time analysis with visible capacity assumptions.

The agent may explain this value and assemble its evidence. It may not promote the value into a broader claim than its evidence class and decision use allow.

---

## M-26 / #26. Illustrative Miami-Dade NAPOT

The worked dashboard displays **3.446 hr/day**. This is classified as `jurisdiction_specific_illustration` rather than being shown as an unexplained number.

### Where the value begins

The lineage begins with DS-01, DS-05, DS-10. The exact stored result is `pump_station_analysis.operating_time.illustrative_Miami_Dade_NAPOT_hours_per_day`. The formula chain is F-PUMP-006 -> F-MDC-NAPOT-001.

This source-to-result path matters because the display may be rounded, reformatted, or combined with a label. The application must always be able to return to the stored result and the frozen input snapshot.

### What the value means and why it matters

Demonstrates the constant-speed rule-pack calculation.

The value is calculated or displayed at this point in the story because a downstream reader needs to understand either the event forcing, the separated I&I response, the pump-station consequence, the operating burden, or the economic scenario.

### What it feeds

No current capacity or compliance determination without applicable legal review.

The agent may explain this value and assemble its evidence. It may not promote the value into a broader claim than its evidence class and decision use allow.

---

## M-27 / #27. Annual modeled I&I reduction

The worked dashboard displays **35.116 MG/yr**. This is classified as `hypothetical_scenario` rather than being shown as an unexplained number.

### Where the value begins

The lineage begins with DS-01, DS-02, DS-03, DS-09. The exact stored result is `rehabilitation_and_economics.annual_total_I_and_I_reduction_MG`. The formula chain is F-ECON-002 -> F-VERIFY-001.

This source-to-result path matters because the display may be rounded, reformatted, or combined with a label. The application must always be able to return to the stored result and the frozen input snapshot.

### What the value means and why it matters

Aggregates assumed dry- and wet-weather reductions.

The value is calculated or displayed at this point in the story because a downstream reader needs to understand either the event forcing, the separated I&I response, the pump-station consequence, the operating burden, or the economic scenario.

### What it feeds

Feeds the narrow direct-cost screen; it is not measured removal.

The agent may explain this value and assemble its evidence. It may not promote the value into a broader claim than its evidence class and decision use allow.

---

## M-28 / #28. Annual gross marginal-cost benefit

The worked dashboard displays **$22,826/yr**. This is classified as `scenario_cost` rather than being shown as an unexplained number.

### Where the value begins

The lineage begins with DS-09. The exact stored result is `rehabilitation_and_economics.annual_gross_marginal_cost_benefit_USD`. The formula chain is F-COST-001.

This source-to-result path matters because the display may be rounded, reformatted, or combined with a label. The application must always be able to return to the stored result and the frozen input snapshot.

### What the value means and why it matters

Values only the declared marginal conveyance and treatment cost.

The value is calculated or displayed at this point in the story because a downstream reader needs to understand either the event forcing, the separated I&I response, the pump-station consequence, the operating burden, or the economic scenario.

### What it feeds

Feeds present value and must show excluded benefits.

The agent may explain this value and assemble its evidence. It may not promote the value into a broader claim than its evidence class and decision use allow.

---

## M-29 / #29. Annual net direct benefit

The worked dashboard displays **-$97,174/yr**. This is classified as `scenario_cost` rather than being shown as an unexplained number.

### Where the value begins

The lineage begins with DS-09. The exact stored result is `rehabilitation_and_economics.annual_net_direct_benefit_USD`. The formula chain is F-COST-001.

This source-to-result path matters because the display may be rounded, reformatted, or combined with a label. The application must always be able to return to the stored result and the frozen input snapshot.

### What the value means and why it matters

Preserves the unfavorable narrow direct-cost result.

The value is calculated or displayed at this point in the story because a downstream reader needs to understand either the event forcing, the separated I&I response, the pump-station consequence, the operating burden, or the economic scenario.

### What it feeds

Prevents the agent from forcing a favorable business case.

The agent may explain this value and assemble its evidence. It may not promote the value into a broader claim than its evidence class and decision use allow.

---

## M-30 / #30. Present value of gross benefits

The worked dashboard displays **$339,586**. This is classified as `scenario_economics` rather than being shown as an unexplained number.

### Where the value begins

The lineage begins with DS-09. The exact stored result is `rehabilitation_and_economics.PV_gross_benefits_USD`. The formula chain is F-PV-001.

This source-to-result path matters because the display may be rounded, reformatted, or combined with a label. The application must always be able to return to the stored result and the frozen input snapshot.

### What the value means and why it matters

Discounts declared annual benefits to the base year.

The value is calculated or displayed at this point in the story because a downstream reader needs to understand either the event forcing, the separated I&I response, the pump-station consequence, the operating burden, or the economic scenario.

### What it feeds

Feeds NPV and benefit-cost ratio.

The agent may explain this value and assemble its evidence. It may not promote the value into a broader claim than its evidence class and decision use allow.

---

## M-31 / #31. Present value of total costs

The worked dashboard displays **$10,285,297**. This is classified as `scenario_economics` rather than being shown as an unexplained number.

### Where the value begins

The lineage begins with DS-09. The exact stored result is `rehabilitation_and_economics.PV_total_costs_USD`. The formula chain is F-PV-001.

This source-to-result path matters because the display may be rounded, reformatted, or combined with a label. The application must always be able to return to the stored result and the frozen input snapshot.

### What the value means and why it matters

Combines capital and modeled recurring costs on the declared basis.
...
## M-32 / #32. Net present value

The worked dashboard displays **-$9,945,711**. This is classified as `scenario_economics` rather than being shown as an unexplained number.

### Where the value begins

The lineage begins with DS-09. The exact stored result is `rehabilitation_and_economics.NPV_USD`. The formula chain is F-PV-001 -> F-ECON-003.

This source-to-result path matters because the display may be rounded, reformatted, or combined with a label. The application must always be able to return to the stored result and the frozen input snapshot.

### What the value means and why it matters

Shows benefits minus costs under the narrow declared boundary.

The value is calculated or displayed at this point in the story because a downstream reader needs to understand either the event forcing, the separated I&I response, the pump-station consequence, the operating burden, or the economic scenario.

### What it feeds

Supports economic screening with excluded benefits shown separately.

The agent may explain this value and assemble its evidence. It may not promote the value into a broader claim than its evidence class and decision use allow.

---

## M-33 / #33. Benefit-cost ratio

The worked dashboard displays **0.033**. This is classified as `scenario_economics` rather than being shown as an unexplained number.

### Where the value begins

The lineage begins with DS-09. The exact stored result is `rehabilitation_and_economics.benefit_cost_ratio`. The formula chain is F-BCR-001.
...
Screens the stated scenario; it does not include omitted public or capacity benefits.

The agent may explain this value and assemble its evidence. It may not promote the value into a broader claim than its evidence class and decision use allow.

---

## M-34 / #34. Simple payback

The worked dashboard displays **Not calculable because annual net benefit is nonpositive**. This is classified as `failed_closed_screen` rather than being shown as an unexplained number.

### Where the value begins

The lineage begins with DS-09. The exact stored result is `rehabilitation_and_economics.simple_payback_years`. The formula chain is F-ECON-004.

This source-to-result path matters because the display may be rounded, reformatted, or combined with a label. The application must always be able to return to the stored result and the frozen input snapshot.

### What the value means and why it matters

Demonstrates that a valid output can be unavailable for a defined mathematical reason.

The value is calculated or displayed at this point in the story because a downstream reader needs to understand either the event forcing, the separated I&I response, the pump-station consequence, the operating burden, or the economic scenario.

### What it feeds

Blocks a false payback claim.
...
A calculation becomes operationally useful only when the application states what decision it may support, which values are required, who must review the result, and what kind of output is allowed.

## DEC-01. Accept or reject event for analysis

This decision consumes M-01, M-03. Those values are assembled because no single metric carries the whole decision. The reviewer needs the connected condition, not one isolated tile.

The required roles are I_and_I_analyst. The allowed output is `accepted_event_or_data_gap`. Naming the output prevents the application from silently turning analysis into a work order, compliance determination, or capital authorization.

Droobi may prepare the evidence packet and draft the allowed output. The named human role remains responsible for acceptance, revision, rejection, and any consequential dispatch.

## DEC-02. Open basin investigation

This decision consumes M-06, M-07, M-08, M-09. Those values are assembled because no single metric carries the whole decision. The reviewer needs the connected condition, not one isolated tile.

The required roles are I_and_I_analyst, collection_system_engineer. The allowed output is `draft_investigation`. Naming the output prevents the application from silently turning analysis into a work order, compliance determination, or capital authorization.

Droobi may prepare the evidence packet and draft the allowed output. The named human role remains responsible for acceptance, revision, rejection, and any consequential dispatch.

## DEC-03. Review station normal and contingency condition

This decision consumes M-10, M-11, M-12, M-13, M-14, M-15, M-16, M-17, M-18, M-19, M-20, M-21. Those values are assembled because no single metric carries the whole decision. The reviewer needs the connected condition, not one isolated tile.

The required roles are pump_station_engineer, operations_supervisor. The allowed output is `approved_contingency_action_or_request_for_more_evidence`. Naming the output prevents the application from silently turning analysis into a work order, compliance determination, or capital authorization.

Droobi may prepare the evidence packet and draft the allowed output. The named human role remains responsible for acceptance, revision, rejection, and any consequential dispatch.

## DEC-04. Review cycling and energy

This decision consumes M-22, M-23, M-24. Those values are assembled because no single metric carries the whole decision. The reviewer needs the connected condition, not one isolated tile.

The required roles are asset_manager, pump_station_engineer. The allowed output is `maintenance_or_efficiency_investigation`. Naming the output prevents the application from silently turning analysis into a work order, compliance determination, or capital authorization.

Droobi may prepare the evidence packet and draft the allowed output. The named human role remains responsible for acceptance, revision, rejection, and any consequential dispatch.

## DEC-05. Review jurisdiction-specific operating time

This decision consumes M-25, M-26. Those values are assembled because no single metric carries the whole decision. The reviewer needs the connected condition, not one isolated tile.

The required roles are operations, compliance_reviewer. The allowed output is `reviewed_rule_pack_finding`. Naming the output prevents the application from silently turning analysis into a work order, compliance determination, or capital authorization.

Droobi may prepare the evidence packet and draft the allowed output. The named human role remains responsible for acceptance, revision, rejection, and any consequential dispatch.

## DEC-06. Screen rehabilitation economics

This decision consumes M-27, M-28, M-29, M-30, M-31, M-32, M-33, M-34. Those values are assembled because no single metric carries the whole decision. The reviewer needs the connected condition, not one isolated tile.

The required roles are engineer, finance, capital_planning. The allowed output is `scenario_review_not_project_authorization`. Naming the output prevents the application from silently turning analysis into a work order, compliance determination, or capital authorization.
...
## Contract 1. Complete operational input and output contract
...
## Contract 2. Formula-by-formula operational catalog
...
## Contract 3. Source-to-input catalog
...
## Contract 4. Numbered dashboard values and complete lineage
...
## Contract 5. Fully populated dashboard mockups
...
# Version 3 status and quality review
