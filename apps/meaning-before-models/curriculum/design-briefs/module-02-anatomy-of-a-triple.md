...
| Working title | The Anatomy of a Triple |
...
| Question | Answer |
| --- | --- |
| What consequential situation opens the lesson? | A sample result is entered as three words, but the direction is reversed and the number has no unit or datatype. |
| What must the learner decide before teaching begins? | Which triple preserves direction and the measured value clearly? |
...
| What should the learner be able to do afterward? | A triple has three positions, but each position has a different job and only the object may be a literal value. |
| What usable work product will the learner create? | Reviewed Ten-Triple Deck |
...
| Element | Selection |
| --- | --- |
| Narrative architecture | A sample result is entered as three words, but the direction is reversed and the number has no unit or datatype. |
...
| Professional work product | Reviewed Ten-Triple Deck |
...
| --- | --- | --- | --- | --- | --- |
| How does IRI apply? | New term | It performs one named job in the module mental model. | A sample result is entered as three words, but the direction is reversed and the number has no unit or datatype. | Selected module visuals | Instructional scenario |
| How does label apply? | New term | It performs one named job in the module mental model. | A sample result is entered as three words, but the direction is reversed and the number has no unit or datatype. | Selected module visuals | Instructional scenario |
| How does literal apply? | New term | It performs one named job in the module mental model. | A sample result is entered as three words, but the direction is reversed and the number has no unit or datatype. | Selected module visuals | Instructional scenario |
| How does datatype apply? | New term | It performs one named job in the module mental model. | A sample result is entered as three words, but the direction is reversed and the number has no unit or datatype. | Selected module visuals | Instructional scenario |
| How does language tag apply? | New term | It performs one named job in the module mental model. | A sample result is entered as three words, but the direction is reversed and the number has no unit or datatype. | Selected module visuals | Instructional scenario |

## Recording script
| Field | Decision |
| --- | --- |
| Script path | `curriculum/scripts/module-02-anatomy-of-a-triple-video-script.md` |
| Intended recording length | 25 to 35 minutes |
| Spoken opening | A sample result is entered as three words, but the direction is reversed and the number has no unit or datatype. |
| Utility example | Module scenario and work product |
| Visual directions | Follow the four selected visual patterns |
| Learner action and work product | Complete Reviewed Ten-Triple Deck |
...
- Work-product format intentionally different: Reviewed Ten-Triple Deck.
