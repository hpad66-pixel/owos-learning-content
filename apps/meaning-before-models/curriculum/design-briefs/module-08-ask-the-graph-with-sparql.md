...
| Field | Decision |
| --- | --- |
| Course and module ID | `owos-course-semantic-data-ai-001`, `mbm001:08` |
| Working title | Ask the Graph with SPARQL |
...
| Question | Answer |
| --- | --- |
| What consequential situation opens the lesson? | A dashboard can count work orders, but operations needs the path from an affected zone to active critical-facility accounts and current procedures. |
| What must the learner decide before teaching begins? | What does a SPARQL query describe first? |
...
| What should the learner be able to do afterward? | SPARQL turns a utility question into variables and triple patterns, then adds filters, optional data, federation, or paths only when needed. |
| What usable work product will the learner create? | Question-to-Query Sheet |
...
| Element | Selection |
| --- | --- |
| Narrative architecture | A dashboard can count work orders, but operations needs the path from an affected zone to active critical-facility accounts and current procedures. |
...
| Professional work product | Question-to-Query Sheet |
...
| --- | --- | --- | --- | --- | --- |
| How does SPARQL apply? | New term | It performs one named job in the module mental model. | A dashboard can count work orders, but operations needs the path from an affected zone to active critical-facility accounts and current procedures. | Selected module visuals | Instructional scenario |
| How does SELECT apply? | New term | It performs one named job in the module mental model. | A dashboard can count work orders, but operations needs the path from an affected zone to active critical-facility accounts and current procedures. | Selected module visuals | Instructional scenario |
| How does variable apply? | New term | It performs one named job in the module mental model. | A dashboard can count work orders, but operations needs the path from an affected zone to active critical-facility accounts and current procedures. | Selected module visuals | Instructional scenario |
| How does triple pattern apply? | New term | It performs one named job in the module mental model. | A dashboard can count work orders, but operations needs the path from an affected zone to active critical-facility accounts and current procedures. | Selected module visuals | Instructional scenario |
| How does FILTER apply? | New term | It performs one named job in the module mental model. | A dashboard can count work orders, but operations needs the path from an affected zone to active critical-facility accounts and current procedures. | Selected module visuals | Instructional scenario |

## Recording script
| Field | Decision |
| --- | --- |
| Script path | `curriculum/scripts/module-08-ask-the-graph-with-sparql-video-script.md` |
| Intended recording length | 25 to 35 minutes |
| Spoken opening | A dashboard can count work orders, but operations needs the path from an affected zone to active critical-facility accounts and current procedures. |
| Utility example | Module scenario and work product |
| Visual directions | Follow the four selected visual patterns |
| Learner action and work product | Complete Question-to-Query Sheet |
...
- Work-product format intentionally different: Question-to-Query Sheet.
