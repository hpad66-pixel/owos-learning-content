...
| Field | Decision |
| --- | --- |
| Course and module ID | `owos-course-semantic-data-ai-001`, `mbm001:04` |
| Working title | From Triples to a Utility Knowledge Graph |
...
| Question | Answer |
| --- | --- |
| What consequential situation opens the lesson? | Operations can see each source record, but the answer requires a path from pump to zone, customer, work order, procedure, and decision owner. |
| What must the learner decide before teaching begins? | What turns isolated triples into a useful graph? |
...
| What should the learner be able to do afterward? | A knowledge graph grows when explicit statements reuse the same identified things and preserve evidence. |
| What usable work product will the learner create? | Utility Mini-Graph |
...
| --- | --- | --- | --- | --- | --- |
| Load source statements | relationship or process | `network-diagram` | Inspect the utility example | A governed boundary becomes visible | multiple-choice |
| Resolve shared nodes | relationship or process | `interactive-process` | Inspect the utility example | A governed boundary becomes visible | multiple-choice |
| Connect paths | relationship or process | `packet-anatomy` | Inspect the utility example | A governed boundary becomes visible | multiple-choice |
| Ask a cross-domain question | relationship or process | `comparison-table` | Inspect the utility example | A governed boundary becomes visible | multiple-choice |

## Module design fingerprint
| Element | Selection |
| --- | --- |
| Narrative architecture | Operations can see each source record, but the answer requires a path from pump to zone, customer, work order, procedure, and decision owner. |
| Mental model | A knowledge graph grows when explicit statements reuse the same identified things and preserve evidence. |
| Purposeful interaction 1 | Four-step mechanism laboratory |
| Purposeful interaction 2 | Governed artifact builder |
| Visual types, minimum four | `network-diagram`, `interactive-process`, `packet-anatomy`, `comparison-table` |
...
| Professional work product | Utility Mini-Graph |
...
| --- | --- | --- | --- | --- | --- |
| Load source statements | network-diagram | Shared component gallery | A knowledge graph grows when explicit statements reuse the same identified things and preserve evidence. | Read from the first boundary to the controlled result | Text guide, conclusion, responsive layout |
| Resolve shared nodes | interactive-process | Shared component gallery | A knowledge graph grows when explicit statements reuse the same identified things and preserve evidence. | Read from the first boundary to the controlled result | Text guide, conclusion, responsive layout |
| Connect paths | packet-anatomy | Shared component gallery | A knowledge graph grows when explicit statements reuse the same identified things and preserve evidence. | Read from the first boundary to the controlled result | Text guide, conclusion, responsive layout |
| Ask a cross-domain question | comparison-table | Shared component gallery | A knowledge graph grows when explicit statements reuse the same identified things and preserve evidence. | Read from the first boundary to the controlled result | Text guide, conclusion, responsive layout |
...
| --- | --- | --- | --- | --- | --- |
| How does node apply? | New term | It performs one named job in the module mental model. | Operations can see each source record, but the answer requires a path from pump to zone, customer, work order, procedure, and decision owner. | Selected module visuals | Instructional scenario |
| How does edge apply? | New term | It performs one named job in the module mental model. | Operations can see each source record, but the answer requires a path from pump to zone, customer, work order, procedure, and decision owner. | Selected module visuals | Instructional scenario |
| How does path apply? | New term | It performs one named job in the module mental model. | Operations can see each source record, but the answer requires a path from pump to zone, customer, work order, procedure, and decision owner. | Selected module visuals | Instructional scenario |
| How does dataset apply? | New term | It performs one named job in the module mental model. | Operations can see each source record, but the answer requires a path from pump to zone, customer, work order, procedure, and decision owner. | Selected module visuals | Instructional scenario |
| How does named graph apply? | New term | It performs one named job in the module mental model. | Operations can see each source record, but the answer requires a path from pump to zone, customer, work order, procedure, and decision owner. | Selected module visuals | Instructional scenario |

## Recording script
| Field | Decision |
| --- | --- |
| Script path | `curriculum/scripts/module-04-from-triples-to-a-utility-knowledge-graph-video-script.md` |
| Intended recording length | 25 to 35 minutes |
| Spoken opening | Operations can see each source record, but the answer requires a path from pump to zone, customer, work order, procedure, and decision owner. |
| Utility example | Module scenario and work product |
| Visual directions | Follow the four selected visual patterns |
| Learner action and work product | Complete Utility Mini-Graph |
...
- Work-product format intentionally different: Utility Mini-Graph.
