# Design Brief for M33: Prompt, Context, Tool Orchestration, and Token Maxing

## Identity

| Field | Decision |
| --- | --- |
| Course and module ID | `one-water-ai-applied-intelligence / legacy:M33` |
...
| Evidence boundary | Blueprint direction only. Do not promise a universal prompt formula or expose private hidden reasoning. Treat model-specific behavior and context limits as dated test results. |

## Learning job

| Question | Answer |
| --- | --- |
| What consequential situation opens the lesson? | A team keeps rewriting one prompt even though the real problem is missing source material, weak permissions, no output contract, and no evaluation step. |
| What must the learner decide before teaching begins? | Decide which failure belongs to the prompt, the supplied context, the surrounding workflow, the source, or the human review design. |
...
| What usable work product will the learner create? | governed prompt and context specification |
...
1. Explain Prompt, Context, Tool Orchestration, and Token Maxing in plain English and connect it to a real water, wastewater, stormwater, or One Water decision.
2. Distinguish prompt, system instruction, context, source, tool, application control layer, output contract, evaluation, and retry without using one term to hide several different jobs.
...
5. Evaluate sources, assumptions, limitations, permissions, and human authority using this boundary: Do not promise a universal prompt formula or expose private hidden reasoning. Treat model-specific behavior and context limits as dated test results.
6. Create a usable governed prompt and context specification with evidence, roles, limits, review steps, and the next accountable action.
...
| --- | --- | --- | --- | --- | --- |
| Core distinctions: prompt, system instruction, context, source, tool, application control layer, output contract, evaluation, and retry | layered structure | prompt-context-control stack | inspect, compare, decide, or build | evidence, consequence, or relationship becomes visible | estimate and explain |
| The module's consequential utility situation | structured explanation | context budget allocator | inspect, compare, decide, or build | evidence, consequence, or relationship becomes visible | matching |
| The learner decision, evidence, and human authority | branching decision | failure diagnosis tree | inspect, compare, decide, or build | evidence, consequence, or relationship becomes visible | decision multiple choice |
| The completed governed prompt and context specification | process and handoff | goal-plan-evidence workflow | inspect, compare, decide, or build | evidence, consequence, or relationship becomes visible | final applied check |
...
| Mental model | A good work order needs a clear task, the right records, approved tools, safety limits, and a closeout check. A prompt alone is only one part. |
| Purposeful interaction 1 | claim challenge, applied to: Decide which failure belongs to the prompt, the supplied context, the surrounding workflow, the source, or the human review design. |
| Purposeful interaction 2 | recommendation builder, producing or reviewing the governed prompt and context specification |
| Visual types | prompt-context-control stack, context budget allocator, failure diagnosis tree, goal-plan-evidence workflow |
...
| Professional work product | governed prompt and context specification |
...
The **governed prompt and context specification** must record the problem, user, decision, approved evidence, assumptions,
...
1. What does **Prompt, Context, Tool Orchestration, and Token Maxing** mean in ordinary utility language?
2. Where can this help in drinking water, wastewater, stormwater, or cross-functional work?
3. What is the difference among prompt, system instruction, context, source, tool, application control layer, output contract, evaluation, and retry?
...
7. What must be present in the **governed prompt and context specification** before it is reviewable?
...
- Previous module reviewed: M32.
- Next module reviewed: M34.
- Opening pattern: begins with the module-specific situation, not a repeated definition panel.
- Dominant visual: prompt-context-control stack.
- Ordered visual-shape sequence: layered structure, structured explanation, branching decision, process and handoff.
