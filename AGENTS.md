...
When a source such as an article, LinkedIn post, video, transcript, conversation, or external framework is being turned into course material, also read and apply `core/standards/SOURCE-TO-CURRICULUM-SPARRING-STANDARD.md`. Do not jump from source extraction to lesson prose. Spar to find Hardeep's original kernel, explain the complete method in plain language, map it backward and forward through the course, design its diagrams and learner actions, and obtain approval on a review package before changing the canonical curriculum blueprint.
