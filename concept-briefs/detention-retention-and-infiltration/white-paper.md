...
```mermaid
flowchart LR
    A[One storm] --> B[Water ledger]
    A --> C[Environmental ledger]
    A --> D[Asset ledger]
    A --> E[Financial ledger]
    A --> F[Public-accountability ledger]
    B --> G[Where did the water go?]
    C --> H[What reached the receiving water?]
    D --> I[Did the assets preserve the intended route?]
    E --> J[What did the event consume, damage, or defer?]
    F --> K[Who was responsible, what was documented, and what changed?]
```
...
```mermaid
flowchart LR
    A[Rainfall] --> B[Surface]
    B --> C[Runoff]
    B --> D[Infiltration at or near the source]
    C --> E[Inlet and conveyance]
    E --> F[Storage or treatment practice]
    F --> G[Controlled surface release]
    F --> H[Soil or engineered media]
    F --> I[Overflow or bypass]
    G --> J[Outfall]
    H --> K[Native soil, groundwater, or underdrain]
    K --> J
    I --> J
    J --> L[Receiving water]
```
...
```mermaid
flowchart TB
    A[The three words people argue about] --> B[Detention: held, then released]
    A --> C[Retention: kept, or just a wet pond, ask which]
    A --> D[Infiltration: crossed into the ground]
    E[The words that let you read a diagram] --> F[Inlet and conveyance: how water arrives]
    E --> G[Storage: where water waits]
    E --> H[Outlet: how water normally leaves]
    E --> I[Overflow and bypass: how water leaves otherwise]
    E --> J[Underdrain: the hidden return route]
    E --> K[Outfall: the last visible point]
```
...
```mermaid
flowchart TB
    A[Runoff enters basin] --> B[Water rises into temporary storage]
    B --> C[Outlet limits or controls release]
    C --> D[Outflow continues after inflow falls]
    D --> E[Basin draws down toward its between-storm condition]
```
...
```mermaid
flowchart LR
    A[Between storms: permanent pool] --> B[Storm inflow arrives]
    B --> C[Pool rises into temporary storage]
    C --> D[Water moves toward controlled outlet]
    D --> E[Water level draws back toward normal pool]
```
...
```mermaid
flowchart TB
    A[Surface water] --> B[Soil or engineered media]
    B --> C[Temporary subsurface storage]
    C --> D[Deeper movement into native soil]
    C --> E[Underdrain]
    C --> F[Plant uptake and evapotranspiration]
    A --> G[Overflow or bypass when capacity is exceeded]
    E --> H[Storm drain or outfall]
    D --> I[Groundwater interaction]
```
...
```mermaid
xychart-beta
    title "Conceptual inflow and controlled outflow"
    x-axis "Time" [0, 1, 2, 3, 4, 5, 6, 7, 8]
    y-axis "Relative flow" 0 --> 10
    line "Inflow" [0, 2, 7, 10, 6, 3, 1, 0, 0]
    line "Outflow" [0, 1, 3, 5, 6, 5, 3, 1, 0]
```
...
```mermaid
flowchart LR
    A[Rainfall] --> B[Stormwater system]
    B --> C[Storage, infiltration, outfall, receiving water]
    A --> D[Surface connection or opening]
    D --> E[Inflow into separate sanitary sewer]
    A --> F[Water enters soil]
    F --> G[Crack, joint, or pipe defect]
    G --> H[Infiltration into separate sanitary sewer]
    E --> I[Wastewater conveyance and pumping]
    H --> I
    I --> J[Wastewater treatment]
```
...
```mermaid
flowchart LR
    A[Clean Water Act framework] --> B[Permit conditions]
    B --> C[Named responsible party]
    C --> D[Program, controls, and maintenance]
    D --> E[Monitoring, inspections, and records]
    E --> F[Reporting and public visibility]
    F --> G[Correction and program improvement]
    G --> H[Protected water and accountable spending]
```
